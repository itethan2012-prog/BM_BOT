...
from MG24xC4 import * ; from xHeaders import *
...
from flask import Flask   # ১ম দিকে import

app = Flask(__name__)     # 👉 এইটাই Flask name (এটা খুব important)
...
📱 Player will see:
"BM ETHAN wants to join your team"
...
async def black666(client_id, key, iv):
    banner_text = "[FF0000][B][C] ERROR , WELCOME TO [FFFFFF]BM ETHAN [00FF00]___ BM ETHAN____ BOT ! \n[FFFF00]NEW VERSION NEW FUNCTION !\n[FF0000]TELEGRAM : @svxbm69\n\n"     
    fields = {
        1: 5,
        2: {
            1: int(client_id),
...
def add_friend(target_uid):
    try:
        # Read credentials from MG24GAMER.txt
        try:
            with open("MG24GAMER.txt", 'r') as f:
...
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Subscribe: [FFFFFF]BM ETHAN [00FF00]!!
"""
...
async def noob(target_uid, chat_id, key, iv, nickname="BMETHAN", title_id=None):
    """EXACT conversion with customizable title ID"""
    try:
        # Use provided title_id or get random one
        if title_id is None:
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg = f"""[B][C][00FF00] Noobde BM ETHAN ya meku agar tu noob bolra toh tu g a y hai
...
"""
    ...
    try:
        for index, title_id in enumerate(all_titles):
            ...
            # You'll need to use your existing title sending logic here
            # For example:
            title_packet = await noob(uid, chat_id, key, iv, nickname="BMETHAN", title_id=title_id)
...
async def auto_reply_with_emote(emote_info, key, iv):
    ...
    try:
        ...
        bot_uid = 14859376920  # Replace with your bot's actual UID
              ...
              # =================== EMOTE DETECTION ONLY ===================
                    ...
                    try:
                        ...
                        if "MĢ24_GÀMER" in WHITELISTED_UIDS:
                            ...
                            try:
                                ...
                                bot_uid = 14859376920
                                message_text = f" Can't accept Your request Talk to BM ETHAN"
                ...
                if insquad == True:
                    try:
                        ...
                        message = """[B][C][FF0000]█▓▒░░ WELCOME TO BM ETHAN BOT ░░▒▓█
[00FF00]»»————-　★　————-««
[FFFFFF]⚡ BOT POWERED BY:[FF0000] BM ETHAN
[FFFFFF]🎯 STATUS: [00FF00]ACTIVE 24/7
[FFFFFF]📱 CONTACT: [FFFF00]@itethan2012@gmail.com
[00FF00]»»————-　★　————-««
...
[b][i][FFC0CB]youtube: BM ETHAN 99[/b]

[b][c][FFC0CB]subcribe: my_channel[FFFFFF]
 
[b][i][FFA500]telegram: @svxbm69[/b]

[b][c][FFA500]telegram contact: @svxbm69[A52A2A]
 
Enjoy the bot my friend.......

[C][B][0000FF] Created by Black666FF
Modified by - BM ETHAN
"""
...
# GLOBAL BLOCKED NAMES
                        BLOCKED_NAMES = ["ETHAN", "Ethan", "BM ETHAN", "ethan", "bm ethan", "Bm Ethan"]  # Protected names
...
# In TcPChaT function, update /status command:
                        ...
                        # FIXED ETHAN TCP HELP MENU SYSTEM - Now detects commands properly
                        # IMPROVED ETHAN TCP HELP MENU SYSTEM - AUTOMATIC MULTI-PART
                        # IMPROVED ETHAN TCP HELP MENU SYSTEM - TREE STYLE FORMAT
                        if inPuTMsG.strip().lower() in ("help","/help","hi","hello","commands"):
                            print(f"Help command detected from UID: {uid}")

                            p1 = """[C][B][FF00FF]━━━━━━━━━━━━━━
[FFFFFF]        ETHAN TCP HELP MENU 1
[FF00FF]━━━━━━━━━━━━━━

[00FF00]01 │ Show Help Menu
[FF0000]   └─ /help

[00FF00]02 │ Get Player Info
[FF0000]   └─ /info [uid]

[00FF00]03 │ Get YouTube Info
[FF0000]   └─ /yt [name]

[00FF00]04 │ Room ID
[FF0000]   └─ /roomid [uid]

[00FF00]05 │ Send Likes
[FF0000]   └─ /like [uid]
...
[FF00FF]━━━━━━━━━━━━━━"""
                            ...
                            p2 = """[C][B][FF00FF]━━━━━━━━━━━━━━
[FFFFFF]        ETHAN TCP HELP MENU 2
[FF00FF]━━━━━━━━━━━━━━

[00FF00]06 │ Leave Group
[FF0000]   └─ /exit
...
[00FF00]09 │ 5 Player Group
[FF0000]   └─ /5

[00FF00]10 │ 3 Player Group
[FF0000]   └─ /3
...
[FF00FF]━━━━━━━━━━━━━━"""
                            ...
                            p3 = """[C][B][FF00FF]━━━━━━━━━━━━━━
[FFFFFF]        ETHAN TCP HELP MENU 3
[FF00FF]━━━━━━━━━━━━━━

[00FF00]11 │ 6 Player Group
[FF0000]   └─ /6

[00FF00]12 │ Ban Player
[FF0000]   └─ /ban [uid]
...
[00FF00]19 │ Add Friend
[FF0000]   └─ /add [uid]

[00FF00]20 │ Remove Friend
[FF0000]   └─ /remove [uid]
...
[FF00FF]━━━━━━━━━━━━━━"""
                            ...
                            p6 = """[C][B][FF00FF]━━━━━━━━━━━━━━
[FFFFFF]        Ethan TCP HELP MENU 6
[FF00FF]━━━━━━━━━━━━━━

[00FF00]26 │ Fast Emote
[FF0000]   └─ /fast [uids] [emote-id]

[00FF00]27 │ Emote Spam
[FF0000]   └─ /es 

[00FF00]28 │ Emote No Bot
[FF0000]   └─ /em [teamcode] [emote]

[00FF00]29 │ Sticker
[FF0000]   └─ /sticker

[00FF00]30 │ Start Match
[FF0000]   └─ /start
...
[FF00FF]━━━━━━━━━━━━━━"""
                            ...
                            p8 = """[C][B][FF00FF]━━━━━━━━━━━━━━
[FFFFFF]        ETHAN TCP HELP MENU 8
[FF00FF]━━━━━━━━━━━━━━

[00FF00]36 │ Guild
[FF0000]   └─ /guild [guild id]

[00FF00]37 │ Spam Join
[FF0000]   └─ /spam_join [uid]

[00FF00]38 │ Spam Req
[FF0000]   └─ /spam_req [uid]

[00FF00]39 │ Spam Invite
[FF0000]   └─ /spm_inv [uid]

[00FF00]40 │ Spam Room
[FF0000]   └─ /spam_room [uid]
...
[00FF00]44 │ Welcome Emote
[FF0000]   └─ /welcomeemote [emoteid]
...
[FF00FF]━━━━━━━━━━━━━━"""
                            ...
                            p12 = """[C][B][FF00FF]━━━━━━━━━━━━━━
[FFFFFF]        ETHAN TCP HELP MENU 12
[FF00FF]━━━━━━━━━━━━━━

[00FF00]56 │ Craftland Badge
[FF0000]   └─ /s1 [uid]
...
[FF00FF]━━━━━━━━━━━━━━"""
                            ...
                            p14 = """[C][B][FF00FF]━━━━━━━━━━━━━━
[FFFFFF]        ETHAN TCP HELP MENU 14
[FF00FF]━━━━━━━━━━━━━━

[00FF00]66 │ Random Emote
[FF0000]   └─ Send Emoji or Title

[00FF00]67 │ Admin Info
[FF0000]   └─ /admin

[00FF00]68 │ DM
[FF0000]   └─ /dm

[00FF00]69 │ Emote Spam
[FF0000]   └─ /es

[00FF00]70 │ Count Emote
[FF0000]   └─ /p
...
[FF00FF]━━━━━━━━━━━━━━"""
                            ...
                            p15 = """[C][B][FF00FF]━━━━━━━━━━━━━━
[FFFFFF]        ETHAN TCP HELP MENU 15
[FF00FF]━━━━━━━━━━━━━━

[00FF00]71 │ Animation List
[FF0000]   └─ /animation

[00FF00]72 │ Play Animation
[FF0000]   └─ /animation [anim]
...
async def MaiiiinE():
    ...
    os.system('clear')
    print("=" * 50)
    print("🤖 ETHANGAMER BOT - INITIALIZING")
    print("=" * 50)
    ...
    os.system('clear')
    print("🤖 ETHANGAMER BOT - CONNECTING")
    print("=" * 50)
    print("┌────────────────────────────────────┐")
    print("│ ██████████████████████████████████ │")
    print("└────────────────────────────────────┘")
